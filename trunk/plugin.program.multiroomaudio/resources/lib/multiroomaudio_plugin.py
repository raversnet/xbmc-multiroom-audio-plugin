...
##	    os.spawnl(os.P_NWAIT,r"del \""+SRCPLS_PATH+""+srcname+"*\"")
##	    os.spawnl(os.P_NWAIT,r"del \""+VIDEO_PATH+""+srcname+"*\"") 	 
##	    os.spawnl(os.P_NWAIT,r"del \""+AUDIO_PATH+""+srcname+"*\"") 	 
...
#############################################################################################################
    def _run_avsource(self, avsourceName):
	    ...
	    xbmc.executebuiltin("Notification(Multiroom Audio,Streamer Stopped,15000,"+DEFAULT_IMG+")")
...
##		    os.spawnl(os.P_NWAIT,r"\""+avsource["application"]+"\" "+avsource["args"]+"")
...
#############################################################################################################
	...
	audio_fc       =  Addon.getSetting( "video_fc" )
	    ...
	    usock    = open( STRTSTRMR_FILE, "w" )
            usock.write("[Diagnostics.Process]::Start(\'taskkill.exe\',\" /IM vlc.exe\")\n")
            usock.write("[Diagnostics.Process]::Start(\'"+vlc_loc+"\',\" -vvv --intf dummy dshow:// --dshow-vdev=none --dshow-caching=10 --sout=#transcode{vcodec=none,acodec=mp3,ab=128,channels=2,samplerate=44100}:"+astrm_type+"{dst="+streaming_ip+":"+streaming_port+"} --sout-rtp-sap --sout-rtp-name="+sap_name+" --sout-standard-sap --sout-standard-name=xbmc_"+sap_name+" --sout-standard-group=Multiroom_AV --file-caching="+audio_fc+"\")\n")
            usock.write("\n")
            usock.close()
	
	    usock    = open( LOOPBACK_FILE, "w" )
            usock.write("[Diagnostics.Process]::Start(\'"+vlc_loc+"\',\" --intf dummy "+vstrm_type+"://@"+streaming_ip+":"+streaming_port+" --fullscreen --file-caching="+video_fc+"\")\n")
            usock.write("\n")
            usock.close()
            ...
            usock.write("\n")
            usock.write("taskkill /IM vlc.exe\n")
            usock.write("\n")
            usock.write("\n")
            usock.write("echo %1\n")
            usock.write("echo So far so good... now Lets Broadcast the stuff were play'n\n")
	    usock.write("\""+vlc_loc+"\" --intf dummy \"%1\" --play-and-exit --extraintf=http --http-host "+localhost+":8084  --sout=#std{access="+vstrm_type+",mux=ts,dst="+streaming_ip+":"+streaming_port+"} --sout-rtp-sap --sout-rtp-name="+sap_name+" --sout-standard-sap --sout-standard-name=xbmc_"+sap_name+" --sout-standard-group=Multiroom_AV --file-caching=300\n")
            usock.write("\n")
	    usock.write("taskkill /IM vlc.exe\n")
	    if (Addon.getSetting( "default_vp" ) == "MR-Video_Stream"):
                usock.write("\n")
                usock.write("echo Restarting Master Streamer\n")
                usock.write("powershell \"& \'"+STRTSTRMR_FILE+"\'\"\n")
	    else:
                usock.close()
	    ...
	    if (Addon.getSetting( "playlocal" ) == "true"):
                ...
                usock.write("taskkill /IM vlc.exe\n")
                usock.write("\n")
                usock.write("Lets start the loopback session\n")
                usock.write("powershell \"& \'"+LOOPBACK_FILE+"\'\"\n")
                usock.write("\n")
                usock.write("echo %1\n")
                usock.write("echo So far so good... now Lets Broadcast the stuff were play'n\n")
	        usock.write("\""+vlc_loc+"\" --intf dummy \"%1\" --play-and-exit --extraintf=http --http-host "+localhost+":8084  --sout=#std{access="+vstrm_type+",mux=ts,dst="+streaming_ip+":"+streaming_port+"} --sout-rtp-sap --sout-rtp-name="+sap_name+" --sout-standard-sap --sout-standard-name=xbmc_"+sap_name+" --sout-standard-group=Multiroom_AV --file-caching=300\n")
	        ...
	        usock    = open( STREAMER, "w" )
                ...
                usock.write("screen -S MAV-Streamer -p 0 -X exec "+vlc_loc+" -vvv --intf dummy --sout=transcode#{vcodec=none,acodec=mp3,ab=128,channels=2,samplerate=44100}:"+astrm_type+"{dst="+streaming_ip+":"+streaming_port+"} --sout-rtp-sap --sout-rtp-name="+sap_name+" --sout-standard-sap --sout-standard-name=xbmc_"+sap_name+" --sout-standard-group=Multiroom_AV --file-caching="+audio_fc+"\n")
                usock.write("fi\n")
                usock.close()
...
#############################################################################################################
    def _save_srcpls (self):
	    ...
	    plsa   = ""+plsa1+plsa2+".pls"
            ...
            if (sys.platform == 'win32'):
##	        os.spawnl(os.P_NWAIT,r"copy \""+DEFAULT_IMG+" "+plsv1+""+plsv2+".tbn\"")
##	        os.spawnl(os.P_NWAIT,r"copy \""+DEFAULT_IMG+" "+plsv1+"default.tbn\"")
##	        os.spawnl(os.P_NWAIT,r"copy \""+DEFAULT_IMG+" "+plsa1+""+plsa2+".tbn\"")
##	        os.spawnl(os.P_NWAIT,r"copy \""+DEFAULT_IMG+" "+plsa1+"default.tbn\"")
    ...
    def	_save_playercorefactoryfile( self ): 
		...
		if (Addon.getSetting( "playlocal" ) == "true"):
		    ...
		    if (Addon.getSetting( "playlocal" ) == "true"):
 		        ...
 		        usock.write("      <args>--intf dummy \"{1}\" --play-and-exit --extraintf=http --http-host "+localhost+":8084 "+video_sout+" --sout=#std{access="+vstrm_type+",mux=ts,dst="+streaming_ip+":"+streaming_port+"} --sout-rtp-sap --sout-rtp-name="+sap_name+" --sout-standard-sap --sout-standard-name=xbmc_"+sap_name+" --sout-standard-group=Multiroom_AV --file-caching="+video_fc+" | "+vlc_loc+" --intf dummy "+vstrm_type+"://@"+streaming_ip+":"+streaming_port+" --file-caching=0 --fullscreen</args>\n")
		    else:
	                usock.write("      <filename>"+vlc_loc+"</filename>\n")
		...
		if (Addon.getSetting( "playlocal" ) == "true"):
		    ...
		    if (Addon.getSetting( "playlocal" ) == "true"):
                	usock.write("      <filename>"+vlc_loc+"</filename>\n")
 		        usock.write("      <args>--intf dummy \"{1}\" --play-and-exit --extraintf=http --http-host "+localhost+":8084 "+audio_sout+" --sout=#std{access="+astrm_type+",mux=ts,dst="+streaming_ip+":"+streaming_port+"} --sout-rtp-sap --sout-rtp-name="+sap_name+" --sout-standard-sap --sout-standard-name=xbmc_"+sap_name+" --sout-standard-group=Multiroom_AV --file-caching="+audio_fc+" | "+vlc_loc+" --intf dummy "+vstrm_type+"://@"+streaming_ip+":"+streaming_port+" --file-caching=0 --fullscreen</args>\n")
		    else:
	                usock.write("      <filename>"+vlc_loc+"</filename>\n")
            ...
            if (sys.platform == 'win32'):
                usock.write("      <filename>"+vlc_loc+"</filename>\n")
		usock.write("      <args>--intf dummy \"{1}\" --play-and-exit --extraintf=http --http-host="+localhost+":8085 --file-caching=0 --fullscreen</args>\n")
            else:
            	if (sys.platform.startswith('linux')):
            	    usock.write("      <filename>"+vlc_loc+"</filename>\n")
 		    usock.write("      <args>--intf dummy \"{1}\" --play-and-exit --extraintf=http --http-host "+localhost+":8085 --file-caching=0 --fullscreen</args>\n")
            ...
            if (sys.platform == 'win32'):
		usock.write("      <args>--intf dummy \"{1}\" --play-and-exit --extraintf=http --http-host="+localhost+":8085 --file-caching=0 --fullscreen --audio-visual visual --effect-list spectrometer</args>\n")
            else:
            	if (sys.platform.startswith('linux')):
 		    usock.write("      <args>--intf dummy \"{1}\" --play-and-exit --extraintf=http --http-host "+localhost+":8085 --file-caching=0 --fullscreen --audio-visual visual --effect-list spectrometer</args>\n")
            	else: 
...
#############################################################################################################
    def _startstrmr( self ):
	    ...
	    if (Addon.getSetting( "mstrrunningflag" ) == "false"):
	        if (sys.platform == 'win32'):
 		    subprocess.call("powershell \"& '"+STRTSTRMR_FILE+"\'\"")
## 		    os.system("powershell \"& '"+STRTSTRMR_FILE+"\'\"")
