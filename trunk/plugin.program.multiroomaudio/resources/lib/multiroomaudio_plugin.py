...
SOURCE_TEMPLATES_FILE_PATH = xbmc.translatePath( os.path.join( "special://profile/addon_data", sys.modules[ "__main__" ].__plugin__, "MultiroomAVsources", "" ) )
SOURCEXX_FILE_TEMPLATE     = xbmc.translatePath( os.path.join( "special://profile/addon_data", sys.modules[ "__main__" ].__plugin__, "MultiroomAVsources", "_dummytmp" ) )
SOURCEXXDATA_FILE_TEMPLATE = xbmc.translatePath( os.path.join( "special://profile/addon_data", sys.modules[ "__main__" ].__plugin__, "MultiroomAVsources", "AV_SourceData" ) )
PCOREXMLDATA_FILE_TEMPLATE = xbmc.translatePath( os.path.join( "special://profile/addon_data", sys.modules[ "__main__" ].__plugin__, "playercorefactory" ) )
...
#############################################################################################################
    def _save_sourceXXfile (self):
        # make settings directory if doesn't exists
        if (not os.path.isdir(os.path.dirname(SOURCE_TEMPLATES_FILE_PATH))):
            os.makedirs(os.path.dirname(SOURCE_TEMPLATES_FILE_PATH));
...
#############################################################################################################
    def _save_sourceXXdatafile (self):
	    ...
	    usock.write("cvlc "+avsource["streamsrc"]+" --fullscreen --file-caching=0\n")
            usock.close()
	    os.system('cp '+SOURCEXX_FILE_TEMPLATE+' '+SOURCE_TEMPLATES_FILE_PATH+''+avsource["srcB"]+'')
...
#############################################################################################################
    def	_save_playercorefactoryfile( self ): 
	    ...
	    audio_fc   =  Addon.getSetting( "audio_fc" )
            ...
            usock.write("      <filename>"+vlc_loc+"</filename>\n")
            usock.write("      <args>--intf dummy \"{1}\" --play-and-exit --file-caching=0 --fullscreen --audio-visual visual --effect-list spectrometer</args>\n")
            usock.write("      <hideconsole>true</hideconsole>\n")
            usock.write("      <hidecursor>true</hidecursor>\n")
            usock.write("      <forceontop>true</forceontop>\n")
            usock.write("    </player>\n")
            usock.write("  </players>\n")
            usock.write("    <rules name=\"replace\">\n")
